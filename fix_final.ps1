$file = "C:\Users\DeLL\OneDrive\Desktop\urja cbg\free-consultation.html"
$content = Get-Content -Path $file -Raw -Encoding UTF8

$unifiedFooter = @"
<footer class="bg-surface-container dark:bg-inverse-surface border-t border-outline-variant dark:border-outline full-width mt-20">
...
if ($content -notmatch '<footer') {
    $content = $content -replace '</body>', "$unifiedFooter`n</body>"
    Set-Content -Path $file -Value $content -Encoding UTF8
    Write-Host "Added footer to free-consultation.html"
} else {
    Write-Host "free-consultation.html already has a footer."
}

# Now standardize the top nav logo across all files
$files = Get-ChildItem -Path "C:\Users\DeLL\OneDrive\Desktop\urja cbg" -Filter "*.html"
$logoSrc = "https://lh3.googleusercontent.com/aida-public/AB6AXuAYy1DWIp0QZCzpYBcwTT-cKLYCSrSiI4cEeLLdjPiyUGA8yh_yQQZeyMCQSUw44ZNJj7vqhP7aRL-QLy7pVheT83JKyWre4Yt4RoVaYfgihsGRhnD1GqC_7B4IZkg1QnGN4VZj-C3FGBZb41155z_cNXrSLM36bCkWE4JSXPiXQPw3xlqMOrGZ8T5kOvdQuFHKXm6fGx0Ee3FYxE4J3BaPrKLn_Rs84mQCFezHgPuPMV27ns50rUCVI8CmS28fQ8kX0rS_3O7h3Acz"

foreach ($f in $files) {
    $c = Get-Content -Path $f.FullName -Raw -Encoding UTF8
    # Replace any aida-public logo src in the nav with the correct one
    # This is a bit tricky, let's just replace all aida-public logo srcs if they look like logos (not large images)
    # The logo usually has alt="...Logo" or is inside the nav. 
    # Let's replace any src="https://lh3.googleusercontent.com/aida-public/[^"]+" where alt contains "Logo"
    
    $c = [regex]::Replace($c, 'src="https://lh3\.googleusercontent\.com/aida-public/[^"]+"([^>]+alt="[^"]*Logo[^"]*")', 'src="' + $logoSrc + '"$1')
    
    Set-Content -Path $f.FullName -Value $c -Encoding UTF8
}
Write-Host "Standardized top nav logos."
